...
our $OPS_EMAIL = 'tim@frontierdigital.com';
...
my $part;
    ...
    unless ($from) {
        $from = getSysParm('MAIL_FROM', $PROGRAMNAME, 'postmaster@frontierdigital.com');
    ...
    unless ($to) {
        unless ($OPS_EMAIL) {
            $OPS_EMAIL = getSysParm('OPS_EMAIL', $PROGRAMNAME, 'postmaster@frontierdigital.com');
